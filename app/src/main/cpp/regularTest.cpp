//
// Created by sunalong on 16/12/28 10:22.
...
int mystrcmpRegular(const char *str1, int length1, const char *str2, int length2) {
    ...
    int res = -1;
    cout << "regular str1:" << str1 << " str2:" << str2 << endl;
    for (int i = 0; i < length1; i++) {
        char c1 = *(str1 + i);
        char c2 = *(str2 + i);
//        cout << "regular  c1 == c2 c1:" << c1 << " c2:" << c2 << " i=" << i << " (length1-1)=" << (length1 - 1) << endl;
        if (i < 3) {//第四个字符才有可能是通配符，前面必须匹配，否则无法决断是哪类机型
            ...
            //todo:如果一直匹配,就不用匹配符
            if (c1 == c2) {
//            if (c1 == c2 && i == (length1 - 1)) {
                cout << "regular  if (c1 == c2) c1:" << c1 << " c2:" << c2 << " i=" << i << " (length1-1)=" << (length1 - 1) << endl;
                if (i == (length1 - 1)) {
                    cout << "regular  i == (length1 - 1) c1:" << c1 << " c2:" << c2 << " i=" << i << " (length1-1)=" << (length1 - 1) <<
                    endl;
                    return 0;
                }
            }else if (*(str2 + length2 - 1) == '*') {
                cout << "regular  }else if (*(str2 + length2 - 1) == '*')  c1:" << c1 << " c2:" << c2 << " i=" << i << " (length1-1)=" << (length1 - 1) << endl;
                if (c2 == '*') {
                    cout << str2 << "字符是通配符，i=" << i << endl;
                    return 0;
                }
            }else{//SM-G9009W str2:SM-G9008W 倒数第二位比较
                cout << "regular -1----- }else if (*(str2 + length2 - 1) == '*')  c1:" << c1 << " c2:" << c2 << " i=" << i << " (length1-1)=" <<
                        (length1 - 1) << endl;
                return -1;
            }
        }
...
//    for (int i = 0; i < phoneModelLength; i++) {
//        cout << "audio_settings " << phoneModel << " length:" << phoneModelLength << " i=" << *(phoneModel + i) << endl;
//    }
    for (int i = 0; i < MODEL_NUM; i++) {
        if (0 == mystrcmpRegular(phoneModel, phoneModelLength, settings[i].modelType, settings[i].modelTypeLength)) {
            cout << "main phoneModel:" << phoneModel << " settings[i].modelType:" << settings[i].modelType << endl;
